...
import {Card, CardContent, Chip, Container, Snackbar, Typography} from '@material-ui/core'
...
const SearchEngine = () => {
    ...
    const [addLike, {data}] = useMutation(LIKE_ARTICLE);
        ...
        if (data) {
            ...
            for (let i = 0; i < articlesToSet.length; i++) {
                if (articlesToSet[i].id === payload.id) {
                    articlesToSet[i] = {...articlesToSet[i], likes: payload.likes}
                }
            }
    ...
    return (
        <Container>
            ...
            />
            <ArticlesList articles={articles} addLike={addLike}/>
            {articles.length === 0 &&
            <Card className={classes.card}>
                <CardContent>
                    ...
                    >
                        Algoritmo para realizar búsquedas optimizadas sobre los <b>reglamentos viales oficiales</b> de
                        tu comunidad.
